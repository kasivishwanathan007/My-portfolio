...
var animateReveal = function () {
	...
	// gsap reveal hero — fires directly (not via ScrollMagic)
	// ScrollMagic can fail to trigger for elements already in the viewport on page load,
	// which causes the hero text to stay invisible on some computers.
	...
	if (grevealhero.length) {
		...
		grevealhero.each(function () {
			...
			// Fire the animation directly after a short delay (page loader is ~200ms)
			setTimeout(function () {
				var tl2 = new TimelineMax();
				tl2
					.to(cover, 1, {
						marginLeft: '0', ease: Expo.easeInOut, onComplete: function () {
							// Set text visible immediately (not through timeline queue)
							revealContent.css({ 'transform': 'none', '-webkit-transform': 'none' });
							// Then slide cover out
							TweenMax.to(cover, 1, { marginLeft: '102%', ease: Expo.easeInOut });
						}
					});
			}, 400 + heroNum * 300);
		...
		// Safety fallback: if GSAP fails or is very slow, force the hero text visible after 4s
		setTimeout(function () {
			$('.gsap-reveal-hero .reveal-content').css({
				'transform': 'none',
